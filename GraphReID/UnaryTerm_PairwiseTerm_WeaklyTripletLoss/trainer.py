...
class Trainer():
    ...
    def full_to_weak(self, x, bagid, batchid, batchimage, num_classes):
        # print(x)
        x = F.one_hot(x, num_classes)
        x = x.view(batchid//bagid, bagid, batchimage, num_classes)
        x = torch.sum(x, dim=1, keepdim=True).type(torch.cuda.FloatTensor)
        x = x / bagid
        # x = x.expand(batchid//bagid, bagid, batchimage, num_classes)
        x = torch.cat([x]*bagid, dim = 1)
        # print(torch.argmax(x[0,0,0,:]), torch.argmax(x[0,1,0,:]) )
    ...
    def train(self):
        ...
        for batch, (inputs, labels) in enumerate(self.train_loader):
            ...
            if inputs.size()[0] != self.args.batchid * self.args.batchimage:
                continue

            self.optimizer.zero_grad()
            outputs = self.model(inputs)

            labels_bag = self.full_to_weak(labels, self.args.bagid, self.args.batchid, self.args.batchimage, self.args.num_classes)
            # labels_deduced = deduce_label(outputs, labels_bag)

            loss = self.loss(outputs, labels_bag)
    ...
    def save_images(self, data, i): 
        # if not os.path.exists(processed_image_path):
        #     os.mkdir(processed_image_path) 
        # one_folder = os.path.join(processed_image_path,relative_folder)
        # if not os.path.exists(one_folder): 
        #     os.mkdir(one_folder)  
        # full_path = os.path.join(one_folder,filename)
        # # restoreed_img_data = tensor_to_PIL(data) 
        unloader_ = transforms.ToPILImage()  
        reverse_mean = [-0.485, -0.456, -0.406]
        reverse_std = [1/0.229, 1/0.224, 1/0.225]  
        # norm_ = transforms.Normalize(reverse_mean, reverse_std)

        # reverse_mean = [-0.485, -0.456, -0.406]
        # reverse_std = [1/0.229, 1/0.224, 1/0.225]
        # recreated_im = copy.copy(im_as_var.data.numpy()[0])
        for c in range(3):
            data[c,:,:] = data[c,:,:]/reverse_std[c]
            data[c,:,:] = data[c,:,:]-reverse_mean[c]
        data[data > 1] = 1
        data[data < 0] = 0
        data = data * 255
        ...
        # write image  
        image.save('/home/wangguangrun/pytorch-image-model/MGN-pytorch/work_dirs/images/' + str(i) + '.jpg')
    ...
    def extract_feature(self, loader):
        ...
        for (inputs, labels) in loader:
            ...
            if flag:
                ...
                for xx in range(10):
                    cc += 1
                    img_to_save = inputs[xx,:,:,:]
                    self.save_images(img_to_save, cc)
                flag = False

            if count%10 ==0:
                print(float(count)/len(loader), len(loader) )
            count += 1
