...
class TripletLoss_full(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        for i in range(n):
            ...
            # print(mask[i], dist[i], dist[i][mask[i]])
...
class TripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # mask = targets.expand(n, n).eq(targets.expand(n, n).t())
        c = targets.size(1)
        targets_a = targets.view(1, -1, c)
        targets_b = targets.view(-1, 1, c)
        mask = targets_a * targets_b
        mask = torch.sum(mask, -1)
        dist_ap, dist_an = [], []
        for i in range(n):
            dist_ap.append(dist[i][mask[i] > 0].median().unsqueeze(0))
            # dist_ap.append(dist[i][mask[i] > 0].median().unsqueeze(0))
            #dist_ap.append(dist[i][mask[i] > 0].max().unsqueeze(0))
            # print(mask[i], dist[i], dist[i][mask[i]])
            dist_an.append(dist[i][mask[i] == 0].min().unsqueeze(0))
        dist_ap = torch.cat(dist_ap)
        dist_an = torch.cat(dist_an)
