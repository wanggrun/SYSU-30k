...
def extract_feature(model,dataloaders):
    ...
    for data in dataloaders:
        ...
        if opt.use_dense:
            ...
            ff = torch.FloatTensor(n,28310).zero_()
        if opt.PCB:
        ...
        #    # outputs = torch.nn.functional.softmax(outputs,-1)
            ...
            ff = ff+f
        #o1 = model(Variable(img.cuda()))
        #f1 = o1.data.cpu()
        #fnorm = torch.norm(f1, p=2, dim=1, keepdim=True)
        #f1 = f1.div(fnorm.expand_as(f1))
        #img = fliplr(img)
        #o2 = model(Variable(img.cuda()))
        #f2 = o2.data.cpu()
        #fnorm = torch.norm(f2, p=2, dim=1, keepdim=True)
        #f2 = f2.div(fnorm.expand_as(f2))
